 ...
 */
public class DateTimeAxis extends ValueAxis<Number> {
  ...
  // -------------- PUBLIC PROPERTIES --------------------------------------------------------------------------------
  /**
   * When true zero is always included in the visible range. This only has effect if
   * auto-ranging is on.
   */
  ...
  private ObjectProperty<TemporalAmount> timeInterval = new SimpleObjectProperty<>(java.time.Duration.ofHours(1));
  ...
  /**
   * The value between each major tick mark in data units. This is automatically set if we
   * are auto-ranging.
   */
  private DoubleProperty tickUnit = new StyleableDoubleProperty(5) {
    ...
    @Override
    public CssMetaData<DateTimeAxis, Number> getCssMetaData() {
  ...
  public DateTimeAxis() {
    this.setForceZeroInRange(false);
  }
   ...
   * @param timeInterval
   */
  public DateTimeAxis(TemporalAmount timeInterval) {
    this.setForceZeroInRange(false);
    this.timeInterval.setValue(timeInterval);
  }

  /**
   * Creates a non-auto-ranging NumberAxis with the given upper bound, lower bound and
   * tick unit.
   *
   ...
   */
  public DateTimeAxis(double lowerBound, double upperBound, double tickUnit) {
    super(lowerBound, upperBound);
    setTickUnit(tickUnit);
  }

  /**
   * Creates a non-auto-ranging NumberAxis with the given lower bound, upper bound and
   * tick unit.
   *
   ...
   * @param tickUnit The tick unit, i.e. space between tickmarks
   */
  public DateTimeAxis(String axisLabel, double lowerBound, double upperBound, double tickUnit) {
    super(lowerBound, upperBound);
    setTickUnit(tickUnit);
    setLabel(axisLabel);
  }
  ...
  /**
   * Called to set the current axis range to the given range. If isAnimating() is true
   * then this method should animate the range to the new range.
   *
  ...
  protected List<Number> calculateTickValues(double length, Object range) {
    ...
    long seconds = this.timeInterval.getValue().get(ChronoUnit.SECONDS);
    lowerBound = Math.floor(lowerBound / seconds) * seconds;
    upperBound = Math.floor(upperBound / seconds) * seconds;
  ...
  /**
   * Measures the size of the label for a given tick mark value. This uses the font that
   * is set for the tick marks.
   *
  ...
  private static class StyleableProperties {
    ...
    private static final CssMetaData<DateTimeAxis, Number> TICK_UNIT
      = new CssMetaData<DateTimeAxis, Number>("-fx-tick-unit", new StyleConverter<Size, Number>(), 5.0) {

      @Override
      public boolean isSettable(DateTimeAxis n) {
        return n.tickUnit == null || !n.tickUnit.isBound();
      }

      @Override
      public StyleableProperty<Number> getStyleableProperty(DateTimeAxis n) {
  ...
  // -------------- INNER CLASSES ------------------------------------------------------------------------------------
  /**
   * Default number formatter for NumberAxis, this stays in sync with auto-ranging and
   * formats values appropriately. You can wrap this formatter to add prefixes or
   * suffixes;
   *
   * @since JavaFX 2.0
   */
  public static class DefaultFormatter extends StringConverter<Number> {
     ...
     */
    public DefaultFormatter(final DateTimeAxis axis) {
     ...
     *
     * @param axis The axis to format tick marks for
     * @param prefix The prefix to append to the start of formatted number, can be null if
     * not needed
     * @param suffix The suffix to append to the end of formatted number, can be null if
     * not needed
     */
    public DefaultFormatter(DateTimeAxis axis, String prefix, String suffix) {
      this(axis);
      this.prefix = prefix;
      this.suffix = suffix;
    }
    ...
    /**
     * Converts the object provided into its string form. Format of the returned string is
    ...
    /**
     * Converts the string provided into a Number defined by the this converter. Format of
     * the string and type of the resulting object is defined by this converter.
     *
